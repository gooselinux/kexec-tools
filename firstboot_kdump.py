...
class moduleClass(Module):
	...
	def createScreen(self, doDebug = None):
		...
		if cmdLine.find("crashkernel") > -1:
			...
			if crashString.find("@") != -1:
				(size, offset) = crashString.split("@")
				self.kdumpMem = int(size[:-1])
				if self.doDebug:
					print "kdumpMem = %d"%self.kdumpMem
				if offset == "0":
					self.kdumpOffset = 0
				else:
					self.kdumpOffset = int(offset[:-1])
				if self.doDebug:
					print "kdumpOffset = %d"%self.kdumpOffset
			else:
		...
		self.memSpin = gtk.Adjustment(self.kdumpMem, lowerBound, upperBound, step, step, 0)
	...
	def apply(self, *args):
		...
		if (self.enableKdumpCheck.get_active() == False):
			os.system("/sbin/chkconfig kdump off")
	...
	def initializeUI(self):
		if not self.configurable:
			self.enableKdumpCheck.set_sensitive(False)
			self.totalMem.set_sensitive(False)
			self.kdumpMem.set_sensitive(False)
			self.systemUsableMem.set_sensitive(False)
			self.labelTotal.set_sensitive(False)
			self.labelKdump.set_sensitive(False)
			self.labelSys.set_sensitive(False)
			self.showErrorMessage(_("Insufficient memory to configure kdump!"))
